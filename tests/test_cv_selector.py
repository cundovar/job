...
def test_recommend_formateur_web_cv_for_training_job():
    job = {
        "title": "Formateur web et développement",
        "description": "Formation adultes, ateliers numeriques, pedagogie et mentorat.",
    }
    recommendation = recommend_cv(job)
    assert recommendation.cv_id == "formateur_web"
    assert recommendation.score >= 3
    assert "formation adultes" in recommendation.matched_keywords
...
def test_recommend_formateur_ia_cv_for_ia_job():
    ...
    assert recommendation.score >= 3
    assert any(kw in recommendation.matched_keywords for kw in ["prompt engineering", "ia générative", "chatgpt"])
